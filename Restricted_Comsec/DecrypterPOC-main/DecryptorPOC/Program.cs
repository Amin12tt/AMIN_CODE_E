...
namespace DecrypterPOC
{
    class Program
    {
        ...
        private static void dropDecryptionLog()
        {
            ...
            ransomWriter.WriteLine(decryptedFileCount + " You file should be good, HEHE :)." +
                "\n----------------------------------------\n" +
                DECRYPTION_LOG);
            ransomWriter.Close();
        }
        ...
        private static void FileDecrypt(string inputFile, string outputFile, string password)
        {
            ...
            {
                Console.WriteLine("Error by closing BankProgress: error: " + ex_CryptographicException.Message);
            }
            catch (Exception ex)
            {
